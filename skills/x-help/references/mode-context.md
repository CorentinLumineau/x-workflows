...
Read each context layer and check availability (5 layers):
...
3. **Agent awareness** — Check if agent-awareness behavioral skill is active (12-agent catalog)
4. **Auto-memory** — Check if `MEMORY.md` is accessible and read last update
5. **Interview state** — Read `.claude/interview-state.json` for confidence history
