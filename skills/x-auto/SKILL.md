...
|--------|-----------|-------------------|
| APEX (Build) | HIGH / CRITICAL | `/x-analyze` -> `/x-plan` -> [APPROVAL] -> `/x-implement` -> `/x-verify` -> `/x-review` -> `/x-commit` |
| APEX (Build) | LOW / MEDIUM | `/x-plan` -> `/x-implement` -> `/x-verify` -> `/x-review` -> `/x-commit` |
| ONESHOT (Fix) | LOW | `/x-fix` -> `/x-commit` |
| ONESHOT (Fix) | MEDIUM | `/x-fix` -> `/x-verify` -> `/x-commit` |
...
Routes to ONESHOT (LOW) -> `/x-fix` -> `/x-commit`. Standard agent.

**Complex feature** - `/x-auto add OAuth2 with Google and GitHub providers`:
Routes to APEX (HIGH) -> `/x-analyze` -> `/x-plan` -> [APPROVAL] -> `/x-implement` -> `/x-verify` -> `/x-review` -> `/x-commit`. Agent: x-designer (opus) for design phase.
