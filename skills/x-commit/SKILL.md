...
3. **Collection directories** (skills, agents, hooks, commands, src, lib, test, tests, components, pages, routes) with 2+ path segments → `{dir1}/{dir2}` group
   - Example: `skills/x-commit/SKILL.md` → group `skills/x-commit`
...
- **Scope**: Last path segment of the group name (e.g., `skills/x-commit` → `x-commit`, `config` → `config`, `root` → project name)
...
|---|-------|-------|----------------|-------|
| 1 | skills/x-commit | 2 | docs | x-commit |
...
Directories that group at the second level (e.g., `skills/x-commit`):
